...
class Candidates(models.Model): #TODO zmienić klucz obcy do Ról kandydatów 
    ...
    Sex = models.CharField(max_length=7, null=True,default=None, choices=SEX_OPTIONS)#=[(tag,tag.value) for tag in sex_choice])
    ...
    Stage = models.CharField(max_length=6,default='',)#choices=[(tag,tag.value) for tag in stage_choice]) 
    Hired = models.CharField(max_length=10,default='',)# choices=[(tag,tag.value) for tag in hired_choice])
...
class Tests(models.Model):
    ...
    Points = models.DecimalField(max_digits = 5, decimal_places = 2,validators=[MaxValueValidator(100),MinValueValidator(0)] )
...
class Calendar(models.Model):
    ...
    Recruitment_Meetings_ID = models.ForeignKey("Recruitment_Meetings", default=None, verbose_name='Worker', null=True, on_delete=models.SET_NULL)
